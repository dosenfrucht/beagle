...
struct __val b_main_b();
struct __val b_fact_b();
void __fun_b_fact_b();
struct __val b_main_b()
{
	__push_num(4);
...
struct __val b_fact_b()
{
	struct __val v = {};
	v.num_args = 1;
	v.data     = __fun_b_fact_b;
	return v;
}
void __fun_b_fact_b()
{
	__push_num(0);
	__push_val(__read_val(1));
	__push_val(builtin__equal__builtin());
	__call(2);
	if (__pop_num(0)) {__push_num(1);} else {__push_num(1); __push_val(__read_val(1)); __push_val(builtin__sub__builtin()); __call(2); __push_val(b_fact_b()); __call(1); __push_val(__read_val(1)); __push_val(builtin__mult__builtin()); __call(2);}
	__unwind(1);
	return;
}
